...
## 권고 (다음 작업)
1. **🔴 즉시 정정**: 코드/문서의 `Luo 2017` → 제거 또는 `Brady et al. 2018`. `D'Mello 2017` → `D'Mello et al. 2014`.
2. **🟡 표현 보정**: Qlarify 인용에 "정신만 차용(확장형 초록 ≠ 하이라이트 질의)" 한 줄 추가. `chat.js` "소크라테스식" 근거 붙이거나 완화.
3. **단일 진실 원천화**: 지금 인용의 출처인 `Private/제미나이 디벨롭 §9~§14`가 레포에 없음 → **이 문서(theory-base.md)를 정본 서지로 삼고**, 코드 주석은 여기로 링크. (또는 원 디벨롭 문서의 서지 부분만 정제해 커밋)
