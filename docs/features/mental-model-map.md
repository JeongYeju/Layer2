...
- **개념 엣지(어휘 중첩)** — `contentTokens()` 가 노드별 흔적(주석·밑줄, 없으면 본문 앞부분)에서 내용 단어를 뽑고(한국어 조사 제거 + 불용어 + 2글자↑), `computeEdges()` 가 단락쌍의 **공유 단어**로 잇는다. df(문서 빈도)로 가중(idf) + 거의 모든 노드에 있는 단어(제목어 등 df>60%) 제외 + 노드당 최대 2개(헤어볼 방지). `drawEdges()` 가 노드 dot 의 y 를 실측해 좌측 거터에 SVG 호(arc)로 그림(굵기·투명도 ∝ 가중치, hover 시 공유 단어 표시). 패널이 숨겨져 있으면 ResizeObserver 가 보일 때 다시 그린다. **임베딩이 아니라 어휘 근사** — 범례에 그렇게 표기.
...
**됐다 (v0.2):** 한 줄 진단, ICAP 막대 + 통계 칩, 척추 노드(ICAP·마찰%·앵커·주석·흔적 수), 훑은 구간 접기, "내가 구성한 개념", **개념 엣지(어휘 중첩 곡선 + 범례)**, 노드 클릭 스크롤, 흔적 없을 때 폴백, 실시간 갱신. 검증 `tests/report.spec.js`(헤드풀 3 pass).

**한계 / 다음:**
- **엣지는 어휘 중첩(근사)** — `gemini-3-flash-preview` 식 의미 임베딩이 아니라 공유 *단어* 기반. 동의어·의역으로 이어진 개념은 못 잡는다. 진짜 의미 연결은 임베딩/LLM 경로(선택)로 추후. (정직성: 범례·문서에 "어휘 중첩"으로 표기.)
