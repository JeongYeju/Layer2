...
상단 토글에 세 번째 뷰가 생겼다 — 스크롤 / 스프레드 / **보드**. 보드 모드로 바꾸면 본문 칼럼이 왼쪽으로 정렬되고, 각 단락 오른쪽에 *그 단락에서 내가 한 일*(밑줄, 동그라미, 주석)이 카드로 펼쳐진다. 흔적이 없는 단락은 작은 점 하나로 접힌다. "해석하기"를 돌려 마찰 계수가 있으면, 치열하게 읽은 단락일수록 왼쪽 테두리가 붉게 물든다.
...
6. 대시보드에서 "해석하기"를 돌렸다면, 마찰이 큰 단락일수록 왼쪽 테두리가 짙은 붉은색.
...
- `frictionByPid()` — `window.__lastInterpretation.refined.paragraphs`에서 `friction_pct`를 읽어 단락에 `--friction-pct` CSS 변수 설정 → 왼쪽 테두리 농도.
...
- ~~마찰 색상은 batch 후에만~~ → ✅ **실시간 시맨틱** (2026-06-03): 보드 진입 시 `refineExport(buildSessionExport())` 로 현재 SignalLog 에서 friction/ICAP 즉시 계산 → 문단마다 *마찰 배경 톤 × ICAP 좌측 보더 색 × 상태 칩*. 해석하기 없이도 시맨틱 뷰. (LLM 해석 결과가 있으면 폴백으로 사용.)
