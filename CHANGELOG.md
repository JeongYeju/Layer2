...
### 멘탈 모델 맵 — 개념 엣지 (v0.2)
- `report.js` 에 **개념 연결선(엣지)** 추가. LLM/임베딩 없이 노드별 흔적(주석·밑줄, 없으면 본문)의 **어휘 중첩**으로 개념이 겹치는 단락을 잇는다: `contentTokens`(한국어 조사 제거+불용어) → `computeEdges`(idf 가중·제목어 제외·노드당 최대 2) → `drawEdges`(노드 dot 실측 → 좌측 거터에 SVG 호, 굵기·투명도 ∝ 가중치, hover 시 공유 단어). 패널 숨김→보임은 ResizeObserver 로 재측정. 범례에 "어휘 중첩"으로 정직 표기(의미 임베딩 아님). 검증 `tests/report.spec.js`(3 pass).
