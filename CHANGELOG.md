...
### 멘탈 모델 맵 — 개념 엣지 (v0.3: 어휘 → 의미 임베딩)
- `report.js` 에 **개념 연결선(엣지)** 추가. 2단계:
  - **어휘(즉시·무료, 기본)** — 노드별 흔적(주석·밑줄, 없으면 본문)의 **어휘 중첩**: `contentTokens`(한국어 조사 제거+불용어) → `computeEdges`(idf 가중·제목어 제외·노드당 최대 2) → `drawEdges`(노드 dot 실측 → 좌측 거터 SVG 호, hover 시 공유 단어).
  - **의미(키 있으면 업그레이드)** — `maybeSemantic` 이 패널 보일 때 노드 원문을 `gemini-embedding-001`(batchEmbedContents)로 임베딩 → **코사인 유사도**로 잇는다. 공유 단어 0인 **동의어·의역**도 연결("디지털 읽기의 마찰"↔"스크린에서 글 읽을 때 느끼는 어려움" cos≈0.77). 임계 = 문서 내 상대(평균+0.3σ)+절대 바닥 0.68. 비용 보호: 보일 때만/`_embCache`/디바운스/노드셋 동일 skip. 범례가 "어휘 중첩" vs "임베딩" 표기.
  - 주의: 의미 모드는 단락 흔적 텍스트를 임베딩 API 로 전송 — 데모/로컬 전용(배포 시 프록시·동의 필요). 키 없으면 어휘로 자동 폴백. 검증 `tests/report.spec.js`(어휘 3 pass) + 키 주입 의미 엣지 수동 검증(범례 "임베딩" 전환·실 API 호출 확인).
